...
- Exception, and the only one: invoking `/implement-prd` **is** that explicit request to create
  `feature/<name>` from clean `main`, then run on that branch — `git add` of implementation files the run
  touched and one or more coherent implementation commits. Nothing under the git-ignored `prd/` directory
  is staged or committed. Nothing else: no push, no `--amend`, no switching to an existing branch, no PR.
  Scope and limits:
